#!/usr/bin/env python3
"""
Script de migration automatique vers Railway avec résolution du problème d'authentification mobile
"""
...
def migrate_database_to_railway():
    ...
    try:
        ...
        if os.path.exists('data_backup_sqlite.json'):
            os.system('python manage.py loaddata data_backup_sqlite.json')
...
def verify_mobile_user():
    """Vérifier que l'utilisateur mobile existe sur Railway"""
    print("\n🔍 Vérification de l'utilisateur mobile...")
    
    try:
        from django.contrib.auth.models import User
        
        # Vérifier si l'utilisateur mobile existe
        try:
            mobile_user = User.objects.get(username='mobile')
            print(f"✅ Utilisateur mobile trouvé (ID: {mobile_user.id})")
...
def main():
    ...
    # Vérification de l'utilisateur mobile
    if not verify_mobile_user():
        print("❌ Problème avec l'utilisateur mobile")
        return 1
