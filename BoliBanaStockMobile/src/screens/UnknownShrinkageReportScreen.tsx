...
export default function UnknownShrinkageReportScreen({ navigation }: any) {
  ...
  const loadShrinkages = async () => {
    try {
      ...
      // Charger toutes les transactions pour filtrer
      const params: any = {
        page_size: 1000,
      };
      ...
      // Filtrer par date
      const filtered = unknownShrinkageTransactions.filter((tx: ShrinkageTransaction) => {
        const txDate = tx.transaction_date || tx.date;
        return isDateInRange(txDate, range.start, range.end);
      });

      // Charger les démarques de l'année précédente pour comparaison
      let previousYearShrinkages: ShrinkageTransaction[] = [];
      try {
        const prevRange = getPreviousYearDateRange(dateFilter);
        const prevFiltered = unknownShrinkageTransactions.filter((tx: ShrinkageTransaction) => {
          const txDate = tx.transaction_date || tx.date;
          return isDateInRange(txDate, prevRange.start, prevRange.end);
        });
        previousYearShrinkages = prevFiltered;
      } catch (error) {
        // Erreur silencieuse
      }
      ...
      calculateStats(filtered, previousYearShrinkages, totalStockValue);
      
      // Calculer le classement des produits
      calculateProductShrinkages(filtered);
    } catch (error) {
      console.error('❌ Erreur chargement démarque inconnue:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const calculateStats = (transactionsList: ShrinkageTransaction[], previousYearShrinkages: ShrinkageTransaction[] = [], totalStockValue: number) => {
    ...
    // Calculer le taux de démarque basé sur la valeur
    ...
    if (previousYearShrinkages.length > 0) {
      ...
      previousYearStats = {
        total_transactions: previousYearShrinkages.length,
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Démarque inconnue</Text>
      ...
      >
        ...
        {loading ? (
          ...
          <>
                ...
                <View style={styles.compactStatsGrid}>
                  <View style={styles.compactStatCard}>
                    <View style={styles.compactStatHeader}>
                      <Ionicons name="swap-horizontal-outline" size={20} color={theme.colors.neutral[600]} />
                      <Text style={styles.compactStatLabel}>Total écarts</Text>
                      {stats.previousYear ? (
                        <View style={styles.inlineComparison}>
                          <Ionicons
                            ...
                            size={10}
                            color={stats.total_transactions >= stats.previousYear.total_transactions ? theme.colors.warning[500] : theme.colors.success[500]}
                          />
                          <Text
                            style={[
                      ...
                      {stats.previousYear ? (
                        <View style={styles.inlineComparison}>
                          <Ionicons
                            ...
                            size={10}
                            color={stats.total_quantity >= stats.previousYear.total_quantity ? theme.colors.warning[500] : theme.colors.success[500]}
                          />
                          <Text
                            style={[
                              styles.inlineComparisonText,
                  ...
                  <View style={styles.compactStatCard}>
                    <View style={styles.compactStatHeader}>
                      <Ionicons name="cash-outline" size={20} color={theme.colors.warning[600]} />
                      ...
                      {stats.previousYear ? (
                        <View style={styles.inlineComparison}>
                          <Ionicons
                            ...
                            size={10}
                            color={stats.total_value >= stats.previousYear.total_value ? theme.colors.warning[500] : theme.colors.success[500]}
                          />
                          <Text
                            style={[
                              styles.inlineComparisonText,
                  ...
                  <View style={styles.compactStatCard}>
                    <View style={styles.compactStatHeader}>
                      <Ionicons name="trending-down-outline" size={20} color={theme.colors.warning[600]} />
                      <Text style={styles.compactStatLabel}>Taux démarque</Text>
                      {stats.previousYear ? (
                        <View style={styles.inlineComparison}>
                          <Ionicons
                            name={stats.shrinkage_rate >= stats.previousYear.shrinkage_rate ? "arrow-up" : "arrow-down"}
                            size={10}
                            color={stats.shrinkage_rate >= stats.previousYear.shrinkage_rate ? theme.colors.warning[500] : theme.colors.success[500]}
                          />
                          <Text
                            style={[
                              styles.inlineComparisonText,
                    ...
                    <Text style={[styles.compactStatValue, { color: theme.colors.warning[600] }]}>
            ...
            {/* Classement des produits avec démarque */}
            {productShrinkages.length > 0 && (
              <View style={styles.productShrinkagesSection}>
                <View style={styles.topProductsHeader}>
                  <Text style={styles.sectionTitle}>🏆 Top produits (démarque)</Text>
                ...
                {productShrinkages.slice(0, 10).map((product, index) => (
                  <View key={product.product_id || product.product_name} style={styles.productRankCard}>
                    ...
                    <View style={styles.productStats}>
                      ...
                      <View style={styles.productStatItem}>
                        <Ionicons name="repeat-outline" size={14} color={theme.colors.warning[500]} />
                        <Text style={[styles.productStatValue, { color: theme.colors.warning[600] }]}>
            ...
            <View style={styles.transactionsSection}>
              <View style={styles.sectionHeader}>
                <Text style={styles.sectionTitle}>
                  Écarts ({filteredTransactions.length})
                </Text>
              </View>

              {filteredTransactions.length === 0 ? (
                <View style={styles.emptyContainer}>
                  <Ionicons name="help-circle-outline" size={48} color={theme.colors.neutral[400]} />
                  <Text style={styles.emptyText}>
                    {searchQuery ? 'Aucun écart trouvé' : 'Aucun écart pour cette période'}
                  </Text>
                </View>
              ) : (
                <FlatList
...
const styles = StyleSheet.create({
  ...
  statsSection: {
    ...
    borderLeftWidth: 3,
    borderLeftColor: theme.colors.warning[300],
    marginHorizontal: 12,
    marginVertical: 8,
    borderRadius: 8,
  },
